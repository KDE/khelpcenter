	...
	while (FindMarker(stream))
	{
		...
		if (!strcmp(buffer, INFO_INDIRECT_TOKEN))
		{
			do							// read the indirects
			{
				...
				if (file == NULL)
				{
					Error.Set(ERR_FATAL, i18n("Error in info file"));
					return 1;
				}
				posPtr = strtok(NULL, ".");
                if ( posPtr )
...
void cTagTable::CreateTable(const char *filename)
{
	...
	if (stream.fail())
	{
		cout << i18n("Cannot open: ") << filename << endl;
		exit(1);
	}
...
void cNodeLineList::AddMenu(const char *buffer)
{
	...
	{
		Error.Set(ERR_WARNING, i18n("Error in info file"));
		return;
	}
...
int cInfo::FindFile(const char *theFilename)
{
	...
	char msg[256];
	sprintf(msg, i18n("Cannot open info file: %s"), theFilename);
	Error.Set(ERR_WARNING, msg);
	return 1;
